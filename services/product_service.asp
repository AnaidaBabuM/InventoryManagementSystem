...
Sub GetProducts(page)
  ...
  If Err.Number <> 0 Then
    Response.Write "Error retrieving products: " & Err.Description
    Response.End
  End If
  On Error GoTo 0
%>
...
Sub SaveProduct(id, name, price, stock, categoryId, supplierId)
  ...
  cmd.Parameters.Append cmd.CreateParameter("price", 5, 1, , CDbl(price))
  cmd.Parameters.Append cmd.CreateParameter("stock", 3, 1, , CInt(stock))
  cmd.Parameters.Append cmd.CreateParameter("category", 3, 1, , CInt(categoryId))
  cmd.Parameters.Append cmd.CreateParameter("supplier", 3, 1, , CInt(supplierId))
  If id <> "" Then
  ...
  cmd.Execute
  If Err.Number <> 0 Then
    Response.Write "Error saving product: " & Err.Description
    Response.End
  End If
  On Error GoTo 0
